...
const unitId = Array.isArray(res.unitId)
            ? res.unitId.map((elem) => ({ _id: elem._id, name: elem.name }))
            : res.unitId
                ? [{ _id: res.unitId._id, name: res.unitId.name }]
                : [];
        setUnitId(unitId);
            })
            .catch((error) => {
...
<div className="col-12 md:col-6 lg:col-3 mb-10"><label className="text-sm text-gray-600">Unit</label>
                    {unitId.map((elem) => (
                        <Link key={elem._id} to={`/unit/${elem._id}`}>
                        <div>
                  {" "}
                            <p className="text-xl text-primary">{elem.name}</p>
                            </div>
