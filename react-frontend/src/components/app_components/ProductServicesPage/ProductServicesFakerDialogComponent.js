...
const [unitIdIds, setunitIdIds] = useState();
...
client.service("unit")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setunitIdIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = productServicesFakerFactory(props.user,fakerCount,branchIdIds,categoryIdIds,unitIdIds);
