...
const [projectId, setProjectId] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [userId,projectId], setError);
        }
        set_entity({...init});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            userId: _entity?.userId?._id,projectId: _entity?.projectId?._id,isActive: _entity?.isActive,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Project Client updated successfully" });
