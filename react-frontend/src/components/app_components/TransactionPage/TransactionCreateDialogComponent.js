...
const TransactionCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            date: _entity?.date,reference: _entity?.reference,description: _entity?.description,branchId: _entity?.branchId?._id,accountId: _entity?.accountId,modelType: _entity?.modelType,modelId: _entity?.modelId,type: _entity?.type,debit: _entity?.debit,credit: _entity?.credit,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Transaction updated successfully" });
