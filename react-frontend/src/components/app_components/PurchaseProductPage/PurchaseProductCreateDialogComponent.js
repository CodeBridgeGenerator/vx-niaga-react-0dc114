...
const [variationId, setVariationId] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [branchId,purchaseId,productId,variationId], setError);
        }
        set_entity({...init});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            branchId: _entity?.branchId?._id,purchaseId: _entity?.purchaseId?._id,productId: _entity?.productId?._id,variationId: _entity?.variationId?._id,quantity: _entity?.quantity,price: _entity?.price,tax: _entity?.tax,discount: _entity?.discount,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Purchase Product updated successfully" });
