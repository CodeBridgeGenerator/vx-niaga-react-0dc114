...
const [categoryId, setCategoryId] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [branchId,customerId,categoryId], setError);
        }
        set_entity({...init});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            branchId: _entity?.branchId?._id,customerId: _entity?.customerId?._id,invoiceDate: _entity?.invoiceDate,categoryId: _entity?.categoryId?._id,status: _entity?.status,source: _entity?.source,deliveryStatus: _entity?.deliveryStatus,description: _entity?.description,invoiceId: _entity?.invoiceId,
