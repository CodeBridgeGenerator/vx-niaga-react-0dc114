...
const [categoryId, setCategoryId] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [branchId,vendorId,warehouseId,purchaseType,categoryId], setError);
        }
        set_entity({...init});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            branchId: _entity?.branchId?._id,vendorId: _entity?.vendorId?._id,warehouseId: _entity?.warehouseId?._id,purchaseType: _entity?.purchaseType?._id,date: _entity?.date,status: _entity?.status,categoryId: _entity?.categoryId?._id,purchaseId: _entity?.purchaseId,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Purchase updated successfully" });
