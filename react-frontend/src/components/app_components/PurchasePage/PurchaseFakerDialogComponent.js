...
const [vendorIdIds, setvendorIdIds] = useState();
const [warehouseIdIds, setwarehouseIdIds] = useState();
const [purchaseTypeIds, setpurchaseTypeIds] = useState();
...
client.service("vendors")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setvendorIdIds(data.data);
                        });
client.service("warehouses")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setwarehouseIdIds(data.data);
                        });
client.service("purchaseType")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setpurchaseTypeIds(data.data);
                        });
client.service("categories")
    ...
    const onRun = async () => {
        let fakeData = purchaseFakerFactory(props.user,fakerCount,branchIdIds,vendorIdIds,warehouseIdIds,purchaseTypeIds,categoryIdIds);
