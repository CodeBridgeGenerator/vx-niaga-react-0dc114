...
const PaymentMethodsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            branchId: _entity?.branchId?._id,name: _entity?.name,accountId: _entity?.accountId,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Payment Methods updated successfully" });
