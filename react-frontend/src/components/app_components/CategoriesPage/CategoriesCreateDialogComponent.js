...
const CategoriesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            branchId: _entity?.branchId?._id,name: _entity?.name,type: _entity?.type,image: _entity?.image,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Categories updated successfully" });
...
<div className="col-12 field">
                <span className="align-items-center">
                    ...
                    <UploadFilesToS3 type={'create'} setValByKey={setValByKey} onSave={onSave} id={urlParams.singleCategoriesId} serviceName="categories" />
                </span>
                <small className="p-error">
