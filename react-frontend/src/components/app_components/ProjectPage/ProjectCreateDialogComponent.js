...
const projectProgressArray = ["Created","InProgress","Completed"];
const projectProgressOptions = projectProgressArray.map((x) => ({ name: x, value: x }));
...
const ProjectCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            title: _entity?.title,type: _entity?.type,startDate: _entity?.startDate,endDate: _entity?.endDate,description: _entity?.description,status: _entity?.status,projectProgress: _entity?.projectProgress,branchId: _entity?.branchId?._id,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Dropdown id="projectProgress" value={_entity?.projectProgress} options={projectProgressOptions} optionLabel="name" optionValue="value" onChange={(e) => setValByKey("projectProgress", e.value)}  />
            </span>
            <small className="p-error">
